package com.rappasocial.webservices;
...
public class DrupalJSONServerNetworkUtilityBase {
    ...
    public static final String BASE_URL = "http://mydancesport.rappasocial.com/dsrm";
...
//        params.put(PARAM_PASSWORD, password);
        ...
        JSONObject json = prepareAndSendHttpPostAuth(AUTH_URI, params);
        ...
        if (json == null) {
        	...
        	try {
        		mSessionCookie = json.getString("session_name") + "=" + json.getString("sessid"); 
    		}
    		catch (JSONException e) {
    			Log.d(TAG, "could not find sessid or session_name from system.connect");
    			return false;
    		}
    ...
    protected static JSONObject prepareAndSendHttpDSRMPost(String URI, String token, String jsondata) {
//    	JSONObject json = null;
//	    try {
//	        json = new JSONObject();
//	        if (params != null) {
...
//	    se.setContentType("charset=utf-8;");
    ...
    protected static JSONObject prepareAndSendHttpPostAuth(String URI, ArrayList<NameValuePair> params) {
    	JSONObject json = null;
	    try {
	        json = new JSONObject();
	        if (params != null) {
	    ...
	    try {
	        ...
	        if (resp.getStatusLine().getStatusCode() == HttpStatus.SC_OK || resp.getStatusLine().getStatusCode() == 406) {
	            if (Log.isLoggable(TAG, Log.VERBOSE)) {
	                Log.v(TAG, "Successful POST");
	            }
	            if (resp.getStatusLine().getStatusCode() == 406){ is406 = true; return null;}
	            return decodeJSONResponse(resp);
	        } else {
	            if (Log.isLoggable(TAG, Log.VERBOSE)) {
	                Log.v(TAG, "Error POSTing: " + resp.getStatusLine());
    ...
    protected static JSONObject prepareAndSendHttpPostBig(String URI, JSONObject json) {
